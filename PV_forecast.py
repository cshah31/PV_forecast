...
def split_dataset(data):
    ...
    train, test = data[0:18864,:], data[18864:21744,:]
    return train, test
...
# Train the LSTM model for Day ahead PV forecast
def train_model(model, train_x, train_y):
    # define parameters
    verbose, epochs, batch_size = 1, 1000, 48
...
normalized_PV_dataset = scale_data.transform(data)
print(normalized_PV_dataset.shape)
#########################################################################################################################################################
# Split the dataset into training and test sets
train, test = split_dataset(normalized_PV_dataset)
print(train.shape)
...
Actual_PV_f = scale_data.inverse_transform(test_y)
Predicted_PV_f = scale_data.inverse_transform(predictions)
print(Actual_PV_f.shape)
print(Predicted_PV_f.shape)

print('Test Mean Absolute Error:', mean_absolute_error(Actual_PV_f[:,0], Predicted_PV_f[:,0]))
print('Test Root Mean Squared Error:',np.sqrt(mean_squared_error(Actual_PV_f[:,0], Predicted_PV_f[:,0])))
...
aa = PV_dataset['Time'].iloc[18864+n_input:18864+n_input+48]
aa = pd.to_datetime(aa, format='%m/%d/%Y %H:%M')
fig, ax = plt.subplots(figsize=(8,4))
plt.plot(aa, Actual_PV_f[:48], marker='.', label="actual")
plt.plot(aa, Predicted_PV_f[:48], 'r', label="prediction")
...
plt.ylabel('PV Active Power (W)', size=15)
